...
@export var camera_x_offset : float = 0.0
...
func _physics_process(_delta): 
	...
	camera_x_offset = (mouse_pos.x - (viewport_dims.x / 2)) * .1
	camera_y_offset = (mouse_pos.y - (viewport_dims.y / 2)) * .1
	var to = player.position + Vector2(camera_x_offset, camera_y_offset)
	var new_pos = self.lerp_overshoot_v(self.position, to, 0.2);
